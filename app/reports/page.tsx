          ...
          <div className="min-w-[240px] relative">
            ...
            <div className="relative">
              <button
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-[12px] text-left hover:border-slate-600 focus:border-sky-500 focus:outline-none transition flex items-center justify-between"
              >
                <span className="text-slate-300">
                  {selectedCount === 0 ? 'Select campaigns...' : selectedCount === campaignOptions.length ? 'All campaigns' : `${selectedCount} campaign${selectedCount > 1 ? 's' : ''}`}
                </span>
              ...
              {dropdownOpen && (
                <div className="absolute z-10 mt-2 w-full bg-slate-900 border border-slate-700 rounded-lg shadow-2xl">
                  <div className="p-2 border-b border-slate-700">
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-[12px] placeholder-slate-600 hover:border-slate-600 focus:border-sky-500 focus:outline-none transition"
                      onClick={(e) => e.stopPropagation()}
                    />
                  </div>
                  <div className="max-h-60 overflow-y-auto p-2">
                    {loadingOptions ? (
                      <div className="text-[12px] text-slate-400 flex items-center gap-2 py-2">
                      ...
                      <div className="text-[12px] text-slate-400 py-2">{searchQuery ? 'No matches found.' : 'No campaigns found.'}</div>
                    ) : (
                      <ul className="space-y-0.5 text-[12px]">
                        {filteredCampaigns.map((c) => (
                          <li key={c.id}>
                            <label className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-slate-800 cursor-pointer transition">
                              <input
                                type="checkbox"
                  ...
                  <div className="flex gap-2 p-2 border-t border-slate-700">
                    <button onClick={() => { selectAll(); }} className="flex-1 text-[11px] px-3 py-1.5 rounded-lg border border-slate-700 bg-slate-950 hover:bg-slate-800 hover:border-slate-600 transition">Select all</button>
                    <button onClick={() => { clearSelection(); }} className="flex-1 text-[11px] px-3 py-1.5 rounded-lg border border-slate-700 bg-slate-950 hover:bg-slate-800 hover:border-slate-600 transition">Clear</button>
                  </div>
                </div>
              )}
            </div>
